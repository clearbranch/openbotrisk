...
**Positioning background: broader ML/cyber sources admitted only to situate bot detection.** A source on broader ML-based cyber detection or security governance — otherwise out of scope under "broader cyber security" and the compliance/governance and general-cyber-book skip rules above — may be admitted **only** as low-proximity positioning background: to situate bot detection as one application of ML-based detection, and to make explicit that its recurring constraints (label availability, feature quality, false-positive handling, feedback loops, human review) are general properties of ML detection rather than bot-specific quirks. Such a source is marked `operational proximity: n/a`, is **non-load-bearing** (it informs explanatory structure, never a claim), and is cited only for the shared detection structure — supervised learning where trustworthy labels exist; unsupervised / anomaly detection and behavioural-baseline (UEBA) methods for deviations from normal. The broader-cyber topics themselves — malware, intrusion, EDR/IR, SOC tooling, incident response, adaptive mitigation, predictive analytics, and governance/compliance as subjects — remain out of scope and are not extracted as content. This narrowly overrides the broader-cyber and general-cyber-book exclusions for the positioning purpose only; it is not a general lane for governance or SecOps material.
